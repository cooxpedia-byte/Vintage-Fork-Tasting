...
describe("Tea Lab workspace", () => {
  ...
  it("offers grouped brewing methods and distinct first and later Gongfu infusion prompts", () => {
    ...
    expect(brewHtml).not.toContain('id="steep-seconds-hours"');
    ...
    expect(brewHtml).toContain('aria-label="Long Infusion, collapsed"');
    ...
    expect(tasteHtml).not.toContain('id="brew-stage-duration-1-hours"');
    ...
    expect(tasteHtml).toContain('aria-label="Infusion time duration selector"');
    expect(tasteHtml).toContain('aria-label="Long Infusion, collapsed"');
