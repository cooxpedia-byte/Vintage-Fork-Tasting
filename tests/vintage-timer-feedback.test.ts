...
describe("vintage timer feedback contract", () => {
  ...
  it("finishes with one sharp, resounding C-G-E bell chord after the mechanical cue", () => {
    expect(VINTAGE_TIMER_COMPLETION_CHIME.bells.map(bell => bell.note))
      .toEqual(["C6", "G6", "E6"]);
    expect(VINTAGE_TIMER_COMPLETION_CHIME.bells.map(bell => bell.frequencyHz))
      .toEqual([1046.502, 1567.982, 1318.51]);
