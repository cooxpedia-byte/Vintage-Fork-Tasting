...
begin
  ...
  if event_id is null then
    ...
    if not public.can_manage_event(event_id,auth.uid()) then raise exception 'not_authorized'; end if;
    if exists(select 1 from public.events where id=event_id and status in ('live','completed','cancelled')) then raise exception 'event_locked'; end if;
    ...
    where id=event_id returning id into saved_id;
    delete from public.event_flight_items where event_id=saved_id;
  end if;
  ...
  if (select count(*) from public.event_flight_items where event_id=saved_id) > 0 then
    update public.events set current_flight_item_id=(select id from public.event_flight_items where event_id=saved_id order by position limit 1) where id=saved_id;
  end if;
