-- Qualify participant columns that conflict with join_event_guest output names.
...
begin
  ...
  if participant_row.id is null then
    ...
    values(
      ...
      case when nullif(trim(p_email),'') is null then null else p_marketing_consent end,
    ...
    update public.participants as existing_participant set
      ...
      marketing_consent=case when nullif(trim(p_email),'') is null then existing_participant.marketing_consent else p_marketing_consent end,
