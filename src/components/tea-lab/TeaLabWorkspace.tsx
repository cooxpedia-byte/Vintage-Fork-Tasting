...
export function BrewStep({ draft, update, back, next }: { draft: TeaLabSoloDraft; update: (recipe: (draft: TeaLabSoloDraft) => TeaLabSoloDraft) => void; back: () => void; next: () => void }) {
  ...
  return <section className="card tea-lab-step">
    ...
    <div className="grid grid-3">
      ...
      <TeaLabDurationSlider key={`initial-${style?.id ?? "seconds"}`} id="steep-seconds" label="Initial steep" valueSeconds={draft.brewing.initialSteepSeconds} preferredUnit={style?.durationUnit ?? "seconds"} enableTimer onChange={value => update(current => ({ ...current, brewing: { ...current.brewing, initialSteepSeconds: value } }))} />
    </div>
