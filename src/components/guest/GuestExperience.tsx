...
type StatePayload = {
  ...
  trivia: null | { id: string; question: string; options: string[]; answerWindowSeconds: number; selectedIndex: number | null; closed: boolean; correctIndex?: number; explanation?: string };
...
export function GuestExperience({ preview, initialParticipant }: { preview: EventPreview; initialParticipant: { id: string; display_name: string } | null }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`event-${preview.id}`, { config: { presence: { key: initialParticipant?.id ?? crypto.randomUUID() } } });
    channel.on("postgres_changes", { event: "UPDATE", schema: "public", table: "event_public_state", filter: `event_id=eq.${preview.id}` }, () => refresh());
    ...
    const heartbeat = window.setInterval(() => { fetch(`/api/events/${preview.id}/heartbeat`, { method: "POST" }).then(r => { if (r.ok) refresh(); }).catch(() => undefined); }, 15000);
    return () => { window.clearInterval(heartbeat); supabase.removeChannel(channel); };
  }, [joined, preview.id, refresh, name, initialParticipant?.id]);
  ...
  useEffect(() => {
    ...
    const delay = Math.max(0, new Date(closesAt).getTime() - Date.now()) + 75;
  ...
  async function answerTrivia(index: number) {
    ...
    const response = await fetch(`/api/events/${preview.id}/trivia`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ selectedIndex: index }) });
    if (response.ok) { const result = await response.json().catch(() => ({ selectedIndex:index })); setTriviaChoice(result.selectedIndex ?? index); if (sound) ceramicClink(); }
    else setError((await response.json().catch(() => ({}))).error ?? "We could not lock that answer.");
  }
  ...
  if (phase === "reveal" && state.currentItem) return <ScheduledReveal state={state} />;
  if (phase === "brewing" && state.currentItem) return <GuestFrame state={state} draft={draft} setDraft={setDraft} sound={sound} toggleSound={toggleSound}><Brewing item={state.currentItem} endsAt={state.event.timer_ends_at} /></GuestFrame>;
...
function ScheduledReveal({ state }: { state: StatePayload }) { const [now,setNow]=useState<number|null>(null); useEffect(()=>{const timer=window.setInterval(()=>setNow(Date.now()),50);return()=>window.clearInterval(timer)},[]); const revealAt=state.event.reveal_at?new Date(state.event.reveal_at).getTime():0; const revealed=!revealAt||(now!==null&&now>=revealAt); return revealed&&state.currentItem?<Ceremony eyebrow={`Tea ${state.currentPosition} of ${state.flightCount}`} title={state.currentItem.reveal_title} subtitle={`${state.currentItem.tea?.origin ?? ""} · ${state.currentItem.reveal_description}`} />:<Ceremony eyebrow={`Tea ${state.currentPosition} of ${state.flightCount}`} title="Ready at the table." subtitle="Your host is revealing the next tea." />; }
...
function Brewing({ item, endsAt }: { item: CurrentItem; endsAt:string|null }) { const [now,setNow]=useState<number|null>(null); useEffect(()=>{const t=window.setInterval(()=>setNow(Date.now()),250);return()=>window.clearInterval(t)},[]); const remaining=endsAt&&now!==null?Math.max(0,new Date(endsAt).getTime()-now):item.steep_seconds*1000; return <><p className="eyebrow">Brewing</p><h1 className="page-title">Brew it like this</h1><p>{item.temperature_c ? `${item.temperature_c}°C` : "Hot water"} · {item.leaf_grams ? `${item.leaf_grams}g` : ""} {item.water_ml ? `per ${item.water_ml}ml` : ""}</p><div className="timer-ring"><div><div className="timer-readout">{formatClock(remaining)}</div><small className="muted">host controlled</small></div></div><section className="card"><p>{item.brewing_instructions}</p></section></>; }
