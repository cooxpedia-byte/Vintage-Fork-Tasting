...
function ClaimButton({ eventId, linked }: { eventId: string; linked: boolean }) {
  ...
  return <><button className="btn btn-primary" disabled={busy} onClick={claim}>{linked ? "Open My Tea Cellar" : busy ? "Saving…" : "Save to My Tea Cellar"}</button><GuestError message={message} /></>;
}
