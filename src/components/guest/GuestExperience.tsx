...
export type StatePayload = {
  ...
  responses: Array<{ event_flight_item_id: string; first_impression: string | null; descriptors: string[]; intensity: string | null; rating: number | null; personal_notes: string | null; saved: boolean; completed_at: string | null; stamp_released_at: string | null }>;
...
export function GuestExperience({ preview, initialParticipant, account, joinRequest = persistGuestJoin }: { preview: EventPreview; initialParticipant: { id: string; display_name: string } | null; account: SignedInAccount; joinRequest?: GuestJoinRequest }) {
  ...
  const videoRoom = soundChosen && state && state.event.location_mode === "remote" && !["recap", "ended"].includes(state.event.phase) && state.event.status !== "completed"
    ? <AgoraVideoRoom eventId={state.event.id} displayName={state.participant.displayName} />
    : null;
  const withPhaseAnnouncement = (content: React.ReactNode) => <><GuestPhaseAnnouncer message={phaseAnnouncement} />{videoRoom}{content}</>;
