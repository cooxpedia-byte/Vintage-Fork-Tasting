...
export function HostConsole({ initialEvent, initialBrew, flight, initialParticipants, userId, userName, userRole }: { initialEvent:EventState; initialBrew:SharedBrew|null; flight:Flight[]; initialParticipants:Participant[]; userId:string; userName:string; userRole:UserRole }) {
  ...
  return <main className="live-shell" id="main-content">
    <div className="live-topbar"><Brand compact /><div><strong>{event.title}</strong><div style={{fontSize:12,opacity:.75}}>{new Date(event.starts_at).toLocaleString("en-CA",{dateStyle:"medium",timeStyle:"short"})}</div></div><span className="spacer"/><span className="chip chip-live">{getConductorStage(conductorStage).label}</span><StatusChip value={event.phase}/><span className="sr-only" role="status" aria-live="polite" aria-atomic="true">{phaseAnnouncement}</span><span className={`chip ${recoveryView.tone === "success" ? "chip-success" : "chip-warning"}`}>{recoveryView.label}</span><span className="chip">{presence} staff device{presence===1?"":"s"}</span></div>
