  ...
  return <form onSubmit={save}>
    ...
    <div className="split event-editor-layout" style={{ marginTop: 20 }}>
      ...
      <aside className="stack">
        <section className="card"><h2 className="card-title">Before launch</h2><ul className="stack" role="list" style={{ gap: 8, marginTop: 12, marginBottom: 0, padding: 0, listStyle: "none" }}>{readiness.map(([label, met]) => <li key={label} className="row" role="listitem"><span aria-hidden="true" style={{ color: met ? "var(--vf-forest)" : "var(--vf-gold)" }}>{met ? "✓" : "⚠"}</span><span>{label}<span className="sr-only">: {met ? "Ready" : "Not ready"}</span></span></li>)}</ul><p className="help" style={{ marginTop: 12 }}>The backend runs the same readiness checks again. The browser never decides launch eligibility by itself.</p></section>
