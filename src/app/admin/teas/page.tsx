...
export default async function TeaLibraryPage() {
  ...
  return <><SiteHeader /><main className="page-shell" id="main-content">
    <div className="row"><div><p className="eyebrow">Permanent records</p><h1 className="page-title">Tea library</h1><p className="page-lede">Manage the defaults used when a tea is added to a future tasting.</p></div><span className="spacer" /><Link className="btn btn-primary" href="/admin/teas/new">+ Add a tea</Link></div>
    ...
    <div className="table-wrap" style={{ marginTop: 20 }}><table><thead><tr><th>Tea</th><th>Producer</th><th>Origin</th><th>Type</th><th>Default steep</th><th>Used in</th><th>Status</th><th>Action</th></tr></thead><tbody>{(teas ?? []).map(tea => <tr key={tea.id}><td><strong>{tea.name}</strong></td><td>{tea.producer || "—"}</td><td>{tea.origin || "—"}</td><td>{tea.tea_type || "—"}</td><td>{tea.default_steep_seconds ? `${tea.default_steep_seconds}s` : "—"}</td><td>{countOf(tea.event_flight_items)} tasting{countOf(tea.event_flight_items) === 1 ? "" : "s"}</td><td><span className={`chip ${tea.retired_at ? "" : "chip-success"}`}>{tea.retired_at ? "Retired" : "Active"}</span></td><td><Link className="btn btn-secondary" href={`/admin/teas/${tea.id}`}>Open</Link></td></tr>)}</tbody></table></div>
