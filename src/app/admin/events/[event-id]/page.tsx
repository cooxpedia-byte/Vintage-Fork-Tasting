...
export default async function EventEditorPage({ params }: { params: Promise<{ "event-id": string }> }) {
  ...
  if (eventId !== "new") {
    const { data: event } = await supabase.from("events").select(`id,title,slug,invite_code,status,location_mode,starts_at,timezone,capacity,venue_name,venue_address,video_call_url,host_user_id,backup_host_user_id,
      flight_items:event_flight_items(tea_id,position,reveal_title,reveal_description,brewing_instructions,steep_seconds,temperature_c,leaf_grams,water_ml,
