...
export default async function LivePage({ params }: { params: Promise<{ "event-id": string }> }) {
  ...
  if (!event) notFound();
  const brewResult=event.current_brew_id
    ? await supabase.from("event_brews").select("id,event_id,event_flight_item_id,infusion_number,started_at,duration_ms,status,paused_at,accumulated_pause_ms,host_id,completed_at").eq("id",event.current_brew_id).maybeSingle()
    : {data:null,error:null};
  if(brewResult.error)logger.warn("host_console_brew_load_failed",{eventId,code:brewResult.error.code});
  return <HostConsole initialEvent={event as never} initialBrew={brewResult.data as never} flight={(flight ?? []) as never} initialParticipants={(participants ?? []) as never} userId={user.id} userName={profile?.display_name ?? user.email ?? "Host"} userRole={role} />;
}
