...
import { CopyEventInviteButton } from "@/components/admin/CopyEventInviteButton";
import { requireStaff } from "@/lib/auth";
import { hostEventPath } from "@/lib/live-events-routes";
    ...
    <section>
      ...
      <div className="table-wrap desktop-admin-table">
        <table>
          ...
          <tbody>{(events ?? []).map(event => <tr key={event.id}><td>{new Date(event.starts_at).toLocaleDateString("en-CA", { month: "short", day: "numeric", year: "numeric" })}</td><td><strong>{event.title}</strong><br /><small className="muted">{event.invite_code ?? "No invite yet"}</small></td><td>{event.location_mode === "remote" ? "Remote" : "In person"}</td><td>{participantCounts.get(event.id) ?? 0} / {event.capacity}</td><td><StatusChip value={event.status} /></td><td><div className="row">{event.invite_code && ["scheduled","live"].includes(event.status) && <CopyEventInviteButton inviteCode={event.invite_code} eventTitle={event.title} />}{["draft","scheduled"].includes(event.status) && <Link className="btn btn-secondary" href={`/admin/events/${event.id}`} prefetch={false}>Edit</Link>}{["scheduled","live"].includes(event.status) && <Link className="btn btn-primary" href={hostEventPath(event.id)} prefetch={false}>{event.status === "live" ? "Resume host room" : "Launch host room"}</Link>}{event.status === "completed" && <Link className="btn btn-primary" href={`/admin/events/${event.id}/results`} prefetch={false}>Results</Link>}{event.status === "cancelled" && <span className="muted">Locked</span>}</div></td></tr>)}</tbody>
        </table>
      </div>
        ...
        {(events ?? []).map(event => {
          ...
          return <li className="mobile-swipe-row" key={event.id} tabIndex={0} aria-describedby={hintId}>
            ...
            <div className="mobile-swipe-actions" role="group" aria-label={`Actions for ${event.title}`}>
              ...
              {event.invite_code && ["scheduled","live"].includes(event.status) && <CopyEventInviteButton inviteCode={event.invite_code} eventTitle={event.title} compact />}
              {["scheduled","live"].includes(event.status) && <Link className="btn btn-primary" href={hostEventPath(event.id)} prefetch={false}>{event.status === "live" ? "Resume host room" : "Launch host room"}</Link>}
