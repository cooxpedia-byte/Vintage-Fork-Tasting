...
export default async function DashboardPage() {
  ...
  const rows = (participants ?? []) as unknown as Array<{ id: string; event_id: string; event: { id: string; title: string; starts_at: string; location_mode: string; status: string; invite_code: string | null }; responses: never[] }>;
  ...
  const upcoming = rows.filter(row => ["scheduled", "live"].includes(row.event.status)).map(row => row.event);
