...
export default async function EventPage({ params }: { params: Promise<{ "invite-code": string }> }) {
  ...
  const participant = await requireParticipant(event.id);
  ...
  return <GuestExperience preview={event as never} initialParticipant={participant ? { id: participant.id, display_name: participant.display_name } : null} />;
}
